...
- `server/api/bots/index.get.ts` (extiende el creado en el spec 03): agrega `actores_mapeados`, el total de actores que ese bot efectivamente recibe (para `alcance='curado'` es el conteo de `bot_actores`; para `alcance='todos'` es el conteo de todos los actores `activo=true`, ya que ese bot los recibe sin curar).
...
- `TablaBots.vue` (nuevo): tabla con columnas zona/nombre/alcance (badge)/medio/institución contratante/activo (badge)/total de actores mapeados (conteo)/acciones.
...
- El `GET /api/bots` se reshapea para incluir `actores_mapeados` (ampliado durante la implementación, ver Decisiones): para `alcance='curado'` es el conteo de filas en `bot_actores` para ese `bot_id`; para `alcance='todos'` es el conteo de `actores` con `activo=true` (todos los que ese bot recibe, sin curar) — nunca se muestra 0 para un bot `todos` con actores activos reales. No se expone la relación cruda de `bot_actores`.
...
3. Extender `server/api/bots/index.get.ts` para incluir `actores_mapeados` (conteo de `bot_actores` si `alcance='curado'`, conteo de `actores` activos si `alcance='todos'`). Prueba manual: `curl http://localhost:3000/api/bots` devuelve `ARGOS LAGUNA` con su conteo de curados y `SINTETIA` con el total de actores activos (no 0).
...
6. Crear `TablaBots.vue`: columnas zona/nombre/alcance (badge)/medio/institución contratante/activo (badge)/"Total de actores mapeados"/acciones. Prueba manual: muestra los bots reales con su conteo correcto (curados para `curado`, total de activos para `todos`).
...
- [x] La constraint `UNIQUE` en `bots.nombre` existe en Mnemosine y `schema.prisma` la refleja tras el `db pull`.
- [x] `GET /api/bots` responde 200 con los bots reales, incluyendo `actores_mapeados` (conteo correcto según `alcance`) por bot.
- [x] Crear un bot nuevo se agrega a la tabla sin recargar.
- [x] Crear un bot con `nombre` duplicado responde 409 legible; crear uno con `zona` duplicada también responde 409 legible, distinguiendo el campo.
- [x] Cambiar el `alcance` de un bot `curado` (con actores curados) a `todos` borra sus filas en `bot_actores`, y `actores_mapeados` pasa a mostrar el total de actores activos (nunca 0, salvo que realmente no haya actores activos).
- [x] Editar un bot (zona, medio, institución contratante, `activo`) persiste el cambio y se refleja en la tabla sin recargar.
- [x] No existe borrado físico de bots.
- [x] `pnpm lint` y `pnpm typecheck` pasan sin errores nuevos (los ya documentados como preexistentes en los specs 01-03 no cuentan).
...
- **Sí (ampliado durante la implementación):** la columna se renombra de "Actores curados" a "Total de actores mapeados", y su cálculo deja de ser un conteo crudo de `bot_actores` — para bots `alcance='todos'` ese conteo siempre daría 0 (nunca tienen filas en `bot_actores` por diseño), lo cual es engañoso: en realidad reciben *todos* los actores activos. Se corrige para que `actores_mapeados` refleje cuántos actores el bot efectivamente recibe: conteo de `bot_actores` si es `curado`, conteo de `actores` con `activo=true` si es `todos`.
